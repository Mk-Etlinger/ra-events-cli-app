...
URL_BASE = "https://www.residentadvisor.net"
  ...
  def get_event_url
    ...
    if EVENTS_BY_STATE.key?(state)
      url = URL_BASE + EVENTS_BY_STATE[state]
    else
      puts "Invalid"
      get_event_url
    end
